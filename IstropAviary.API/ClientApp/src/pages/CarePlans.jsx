...
const CarePlans = () => {
  const { carePlans, setCarePlans } = useData();
  ...
  // Generate 14-day template
  const cycleDays = Array.from({ length: 14 }, (_, i) => {
    const dayNum = i + 1;
  ...
  const activeDayData = cycleDays.find(d => d.dayNumber === selectedDay);

  const handleAddOrUpdatePlan = (planData) => {
    if (planData.id) {
      // Update existing
      setCarePlans(prev => prev.map(p => p.id === planData.id ? planData : p));
    } else {
      // Add new
      setCarePlans(prev => [...prev, { ...planData, id: Date.now() }]);
    }
    setEditingItem(null);
  };
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto pb-10">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-stretch">
        ...
        <div className="w-full lg:w-auto bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col shrink-0 print:hidden">
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-6">
            
            {/* 1. Hafta */}
            <div className="space-y-1 min-w-[240px] xl:min-w-[260px]">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">1. Hafta</h4>
              {cycleDays.filter(d => d.weekNum === 1).map((day) => (
                <div 
                  key={day.dayNumber}
            ...
            {/* 2. Hafta */}
            <div className="space-y-1 min-w-[240px] xl:min-w-[260px]">
              <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">2. Hafta</h4>
              {cycleDays.filter(d => d.weekNum === 2).map((day) => (
                <div 
                  key={day.dayNumber}
                  onClick={() => setSelectedDay(day.dayNumber)}
                  className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all ${
